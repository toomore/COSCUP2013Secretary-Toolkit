    ...
    with open(path, 'r+') as files:
        ...
        for r in csv_reader:
            ...
            if r['status'] == '拒絕':
                ...
                if r['start_date']:
                    r['expiration_date'] = arrow.get(
                        r['start_date']).shift(years=1).format('YYYY-MM-DD')
...
def send_pycon_token(rows, dry_run=True):
    template = TPLENV.get_template('./pycon_2024_token.html')
    _n = 1
    for u in rows:
        if u['mail'] in setting.BLOCK:
            continue
        ...
        raw = make_raw_email(
            nickname=u['name'],
            mail=u['mail'],
            subject=u'[OSCVPass][提醒] PyConTW 2024 (%s)' % u['name'],
            body=body,
            dry_run=dry_run,
        )
        SENDER.client.send_raw_email(RawMessage={'Data': raw})
...
def send_hitcon_token(rows, dry_run=True):
    ...
    for u in rows:
        ...
        raw = make_raw_email(
            nickname=u['name'],
            mail=u['mail'],
            subject=u'[OSCVPass][提醒] HITCON 2024 (%s)' % u['name'],
            body=body,
            dry_run=dry_run,
        )
        SENDER.client.send_raw_email(RawMessage={'Data': raw})
...
def send_lv_token(rows, dry_run=True):
    ...
    for u in rows:
        ...
        raw = make_raw_email(
            nickname=u['name'],
            mail=u['mail'],
            subject=u'[再提醒] [OSCVPass] Laravel x Vue Conf Taiwan 2023 優惠券 (%s)' % u[
                'name'],
            body=body,
            dry_run=dry_run,
        )
    ...
    with open(path) as files:
        for u in csv.DictReader(files):
            ...
            if u['mail'] not in datas:
                datas[u['mail']] = {
                    'nickname': u['nickname'], 'mail': u['mail']}
...
if __name__ == '__main__':
    ...
    #   send_hitcon_token(rows=rows, dry_run=True)
